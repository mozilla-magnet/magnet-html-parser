...
var debug = require('debug')('open-graph');
...
module.exports = function($) {
  var data = _parseOpenGraphDoc($);
  var result = {};

  debug('Existing: ', result.data);
  if (!Object.keys(data).length) return Promise.resolve(result);
...
(function init() {
  for (const key in openGraphTypes) {
...
function _parseOpenGraphDoc($) {
  ...
  function startsWithPrefix(value) {
    for (let prefixIndex = 0; prefixIndex < prefixChecks.length; prefixIndex++) {
      if (value.startsWith(prefixChecks[prefixIndex])) return true;
    }
  ...
  metaTags.each((i, tag) => {
    var $tag = $(tag);
    const property = $tag.prop('property');
    ...
    if (property && startsWithPrefix(property)) {
      ...
      if ($tag.prop('content')) {
        value = $tag.prop('content');
      } else if ($tag.attr('value')) {
      ...
      if (key === 'type') {
        ...
        if (value in openGraphTypes) {
          debug('Adding new opengraph type', Object.keys(openGraphTypes[value]));
          pageType = value;
          ...
          if (value in typeAliases) {
            ...
            debug(`Unknown opengraph type: ${value}`);
          }
        }
      }
  ...
  debug('Page opengraph data: ', page);
  return page;
}
